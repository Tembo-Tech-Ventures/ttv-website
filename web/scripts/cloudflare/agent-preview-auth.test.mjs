...
describe("agent preview identity", () => {
  it("accepts bounded agent names and rejects ambiguous resource names", () => {
    ...
    expect(() => assertAgentEnvironmentName(`agent-${"a".repeat(40)}`)).toThrow(
      "at most 40"
    );
    expect(() => assertAgentEnvironmentName("agent-trailing-")).toThrow("may not end");
  });
  ...
  it("derives separate, environment-specific session and auth secrets", () => {
    ...
    expect(deriveAgentPreviewAuthSecret(secret, "agent-pr-55")).not.toBe(firstToken);
  ...
  it("seeds an expiring isolated admin without exposing the token", async () => {
    ...
    expect(executeQuery.mock.calls[4][2]).toContain(AGENT_PREVIEW_SESSION_MARKER);
