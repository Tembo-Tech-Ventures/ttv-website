...
const lines = [
  "-- Generated by import-data.mjs",
  `-- Source exported at: ${data.exportedAt}`,
  "",
];
...
for (const u of data.users ?? []) {
  lines.push(
    `INSERT OR IGNORE INTO \`user\` (id, name, email, emailVerified, image) VALUES (${esc(u.id)}, ${esc(u.name ?? "")}, ${esc(u.email)}, ${boolInt(u.emailVerified)}, ${u.image !== null && u.image !== undefined ? esc(u.image) : "NULL"});`,
  );
}
lines.push("");
...
for (const pa of data.programApplications ?? []) {
  ...
  lines.push(
    `INSERT OR IGNORE INTO \`programApplication\` (id, programId, userId, partnerId, status, application, completedAt, createdAt, updatedAt) VALUES (${esc(pa.id)}, ${pa.programId !== null && pa.programId !== undefined ? esc(pa.programId) : "NULL"}, ${esc(pa.userId)}, ${pa.partnerId !== null && pa.partnerId !== undefined ? esc(pa.partnerId) : "NULL"}, ${esc(pa.status)}, ${esc(appJson)}, ${ts(pa.completedAt)}, ${ts(pa.createdAt)}, ${ts(pa.updatedAt)});`,
  );
}
lines.push("");
